...
void updateU(double *U,int Nx,double dx,double dt,int Nt){
    ...
    for(N = 1;N<=Nt;N++){
        ...
        for(j=2;j<Nx-2;j++){
            for(i=0;i<3;i++){
                Unext[i][j] = 2.0*U[j+i*Nx]/3.0 + 1.0/3.0*(U2[i][j]- cfl*(fo[i][j-1]-fo[i][j-2]));
            }
        }
        for(i=0;i<3;i++){
            for(j=0;j<Nx;j++){
...
int main(){
    ...
    double *invS = new double[9];
    Su(u,S);
    invSu(u,invS);
    mul_matrix33(S,u,C[0],3,3,1);
    print_matrix(C[0],1,3);
    print_matrix(S);
    //print_matrix(invS);
    //print_matrix(C[0]);
    ...
    for(int i=0;i<Nx;i++){
        //cout << i*dx << endl;
        U[0][i] = (i>Nx/2 ? 1 : 0.125);
        U[1][i] = 0.;
        U[2][i] = (i>Nx/2 ? 1 : 0.1)/(g-1);
    }

...
//差分格式一阶迎风
void first_windward(double *f,double &fm) {fm = f[0];}
...
void Flux_e(double *U,int Nx,double *fo,int bpn,int sn,int bopn){
    ...
    for(j=bopn;j<Nx-1-bopn;j++){
        ...
        for(i=0;i<3;i++) {first_windward(fhp[i],fhpm[i]);first_windward(fhm[i],fhmm[i]);} //ft1 = fhat^+
...
int main(){
    ...
    updateU(U[0],Nx,dx,dt,Nt,1,0);  //使用一阶迎风，一个基架点

    //输出
    ofstream csvfile;
    csvfile.open("General_SW-firstwindward_t=0.14.csv", ios::out | ios::trunc);
